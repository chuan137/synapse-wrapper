...
6. **worktree 隔离** —— `spawn` 走 spec §1.3 的 `dirtyStrategy`,落地 `backend/worktree.ts`。`--strategy require-clean|ignore|carry-stash`。**优先级已升**:第 5 步上线后,两个任务的主 agent 并行跑就会让子 agent 的改动缠进同一工作树、没法按任务提交(实测,见 `../notes/implementation-lessons.md`「并行主 agent 共用同一工作树」)。可先做一个薄版本止血 —— `bin/agent.ts` 加 `--worktree`:后端 `spawn` 时 `git worktree add ~/.synapse/worktrees/<project>-<taskId>-<bindingId 前缀>`(默认 `ignore` 策略,从干净 `HEAD`),把该路径当 `--workspace` 传给子 agent;`DELETE /agents/:bindingId` 或任务归档时 `git worktree remove`。完整 `dirtyStrategy` / `linkFiles` / `carry-stash` 仍按 §1.3 后续补。
