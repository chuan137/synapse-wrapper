...
- ~~**`turn_completed` 摘要的来源**~~ —— ✅ 落地顺序第 5 步已做一半。`turn_end` 的 `result`(最终 assistant 文本)+ `costUsd` 带进 `TaskEvent.data`,`emitTaskEvent(…, 'turn_completed', …)` 的第 4 参;`agentContext()` / `synapse agent poll` / `await` 都从 `data.result` 取真结论,message 仍是固定串「轮次完成 / 轮次已中断」。**未做**:改动文件列表 —— `turn_end` 载荷里没有,要从子 agent 的 transcript 解析(`backend/transcript.ts`),留待后续。
...
4. ~~**主 agent 受限 settings**(见「约束主 agent 的行为」)~~ —— ✅ 已完成。`daemon.ts` 新增 `writeMainAgentSettings()` / `mainAgentSettingsPath()`,写 `<数据目录>/main-agent.settings.json`(`0600`,只有 `permissions`:Bash 白名单 `synapse agent:*` + git 只读子命令 + `ls`/`cat`/`rg`/`find` + `Read(**)`;`deny` Write/Edit/NotebookEdit/Task/WebFetch/WebSearch),`server.ts` 监听成功后挨着 `writeHookSettings` 写一遍(内容不含端口,幂等)。`CreateOptions` 新增 `settingsPaths?: string[]`(没改 `settingsPath` 那个字段名 —— 会话的共用 hook 路径仍叫它;`settingsPaths` 是叠加的额外路径),`TmuxOptions` / `StreamJsonOptions` 加 `extraSettingsPaths`,两个 transport 都 `flatMap(p => ['--settings', p])` 追加在共用那份之后。`role:'main'` 分支传 `settingsPaths: [mainAgentSettingsPath()]`。共用 `hooks.settings.json` 仍只放 `hooks`。主 agent system prompt 补了「工具被限制过,白名单外 Bash 会卡住,不要绕过」。
   - **解绑 kill-session** —— ✅ `DELETE /api/tasks/:id/agents/:bindingId` 判 `role:'main' && transportKind:'tmux' && s.tmuxName === 'synapse-main-<taskId>' && paneId == null`,是则 `manager.stop(localId, true)`(透传 `killSession` 到 `TmuxTransport.stop(true)` → `tmux kill-session`)。`SessionTransport.stop` 签名加可选 `killSession?: boolean`;`manager.stop(localId, killSession=false)` 透传。接管 pane 的普通会话解绑仍只断绑定不碰会话。
   - **重启扫回** —— ✅ `SessionManager.#reclaimOwnTmuxSessions()`(挨着已有的 `#reclaimTmuxSessions` pane 扫回):按会话名 `has-session -t <name>`(`tmuxSessionExists()`)判自建 tmux 会话在不在,在就用会话名重建 `TmuxTransport`(`start()` 里 `#sessionExists` 命中跳过创建,只 `#waitReady` + 接管转写)。会话没了维持 exited。`SessionManager.reclaimDone` promise 暴露扫回完成时机;`server.ts` 的 `reconcileMainAgentsAfterRestart()` await 它之后,对仍 exited 的 `role:'main'` tmux binding `detachAgent` + 写 `agent_exited` 事件。**偏差**:设计写的是「`daemon.ts` 启动路径 `tmux list-sessions`」,实际落在 `SessionManager` 里复用现有 tmux 扫回机制 —— 自建 tmux 会话本来就是 persisted session,`#loadPersisted` 已加载,只差按名重连这一步,不必在 daemon 层另拉一次 `list-sessions`。顺带修了一个潜在缺口:自建 tmux 会话(不只是主 agent)此前重启后从不扫回,一律显示 exited。
     **连带修的前置 bug**:光有扫回不够 —— 实测发现旧进程 `shutdown()` → `manager.stopAll()` 会对**所有**会话(含 tmux)调 `#markExited`,而 `#markExited` 会 `#emit('session_updated', exited)`,`server.ts` 的 `endBindingForExited` 监听器据此在旧进程退出前就把 main binding 的 `endedAt` 落盘了 —— 新进程的扫回再怎么把 tmux 会话接回来,binding 已经断了,补不回去(这正是 `daemon-restart-unbinds-agents` 那条已知限制的根因)。修法:`stopAll()` 对 `transportKind === 'tmux'` 的会话跳过 `#markExited`(那个进程不随 daemon 退出,标 exited 是误报;`#persistedAll()` 不落 `state` 字段,不影响下次启动的 `#loadPersisted` 判断)。手动验证:main agent 存活时重启 → binding 保持 `endedAt: null`、`state: ready`,任务流不出现 `agent_exited`;重启窗口内额外 kill 掉 tmux 会话 → 才出现预期的 `agent_exited` + `detachAgent`。
5. ~~**`spawn` + `poll` + `await`**~~ —— ✅ 已完成。`spawnSubAgent()` 从 `/agents/start` 的 sub 分支抽出,与新端点 `POST /api/tasks/:id/agents/spawn` 两处共用;`spawn` 的 `--handoff <file>` 由 **CLI 侧** `readFileSync` 读出内容放进 body,后端 `spawnFirstPrompt()` 用它替换 `subAgentPrompt()` 模板拼首轮 prompt,`send()` 后返回 `{ bindingId, workspace }`(NDJSON 一行)。`GET /api/tasks/:id/agents/:bindingId/events?since=` 直接用 `tasks.eventsForBinding()`,无新事件立即返回 `{ events: [], cursor: since }`,不 hang。`bin/agent.ts` 的 `poll <bindingId> --since` 输出每事件一行 NDJSON + 末行 `{"cursor"}`;`await <bindingId> --timeout` 是 CLI 侧 2s 轮询循环(单次失败指数退避、连续 5 次判 `20`,退出码 `0/10/11/20`)。`turn_end` 的 `result` / `costUsd` 带进 `TaskEvent.data`,`agentContext()` 的 `lastTurnSummary` 改从 `data.result`(首行、截 200 字)取。**偏差 / 未做**:改动文件列表 `turn_end` 载荷没有,留待从 transcript 解析(见「未决」);`--strategy`(worktree 隔离)后端收下即忽略,留给第 6 步;`AgentBinding.parentBindingId` 与 `subagent_dispatched` / `subagent_returned` 事件未加(现有 `agent_started` / `turn_completed` / `agent_exited` 已够 `poll` 用);跳过了第 4 步(主 agent 受限 settings),当前主 agent 仍有全套工具。
6. ~~**worktree 隔离(薄版本)**~~ —— ✅ 已完成。`bin/agent.ts` 的 `spawn` 加布尔 flag `--worktree`(`parseFlags` 加 `BOOL_FLAGS`),原先收下即忽略的 `--strategy` 删掉。后端 `backend/worktree.ts`:`repoRootOf()` 反查仓库根(非 git 抛错回主 agent),`addWorktree()` 从干净 `HEAD` `git worktree add --detach ~/.synapse/worktrees/<projectSlug>-<taskId>-<binding 前缀>`(等价 §1.3 `ignore` 策略),`removeWorktree()` 从 worktree 反查 `--git-common-dir` 得主库根后 `git worktree remove --force`。`AgentBinding.worktreePath` 记路径(旧 binding 加载时补 `null`);`spawnSubAgent()` 预生成 binding id 供 worktree 命名 + 注入 + attach 共用。`spawn` 端点 body 加 `worktree: boolean`,返回的 `workspace` 是 worktree 路径。`DELETE /agents/:bindingId` 与 `PATCH /api/tasks/:id`(status → `archived`)对有 `worktreePath` 的 binding `removeWorktree`。主 agent system prompt 补「spawn 务必带 `--worktree`」+ 顺带把 spawn/poll/await 用法从「后续开放」更新为已开放。**未做**(留 §1.3):`dirtyStrategy` 三策略选择、`linkFiles` 搬运、`carry-stash`、clone 变体、网页 UI 直接启动的子 agent 接 `--worktree`、显式「移除 worktree」入口(当前跟随解绑 / 归档自动删)。
7. ~~**`backend/taskDocs.ts` + `synapse-tasks` repo**~~ —— ✅ 已完成。`backend/taskDocs.ts`:`tasksRepoRoot()`(`SYNAPSE_TASKS_REPO` 覆盖,默认 `~/gb/synapse-tasks`)、`ensureRepo()` 首次写入前 `git init` + 落 `CLAUDE.md` + `.claude/skills/synapse-handoff/SKILL.md`(「skill 初版」原文,commit 失败静默吞掉 —— 用户机器 git 未配 `user.name`/`email` 时文件仍落盘,只是没提交上)、`readDoc`/`writeDoc`/`appendChangelog` 三个读写函数、`ensureTasksRepo()` 供起主 agent 前预建。端点:`GET/PUT /api/tasks/:id/docs/:kind`(`:kind` 白名单 `handoff|progress|changelog`,`PUT` 只接受 `handoff`/`progress`)、`POST /api/tasks/:id/docs/changelog`(append-only)。`bin/agent.ts` 的 `doc <kind>` 子命令:读 stdin(非 TTY 才读),有内容 → 写(`changelog` 走 POST,其余 PUT),无内容 → GET 打印。`role:'main'` 分支起会话前 `ensureTasksRepo()` 拿到路径,`CreateOptions.addDir?: string[]` 新增(`TmuxTransport`/`SessionManager.create` 透传成 `--add-dir`,仅自建会话生效),传 `addDir: [tasksRepo]`。`mainAgentPrompt()` 补 `synapse agent doc` 用法 + 「写前先加载 synapse-handoff skill」+ 开场提示改为「先 `context` 再 `doc handoff` 再拆解」。**路径不穿越**:目录由 `taskId`(UUID,daemon 生成不由主 agent 传)+ `projectSlug(Project.name)` 拼出,不接受调用方传入的相对路径片段;`isSafeTaskDir()` 留作后续校验的兜底(当前调用路径 taskId 恒为受信来源,未强制调用)。**未做**:`renderState()`(标记区渲染,留给第 8 步)、`AgentBinding.worktreePath` 之外还没有把 changelog 与 `spawn`/`turn_completed` 事件自动关联(主 agent 手动调 `doc changelog` 记)。
