...
一句话:调度用 `synapse agent {context,spawn,poll,await,doc}` 子命令(daemon HTTP 的瘦客户端,每次调用是一次短请求、不 hang 长连接,轮询逻辑在 CLI 侧),不引入 MCP;主 agent 沉淀的 handoff / progress / changelog 落一个独立 git repo(`backend/taskDocs.ts`,`synapse agent doc` 已实现)。**主 agent 由网页直接启动**,走 `TmuxTransport` 自建会话(`synapse-main-<taskId>`,长命、扛 `daemon restart`);行为靠 Claude Code settings 层 `permissions` 约束(`daemon.ts` `writeMainAgentSettings()` 写的 `main-agent.settings.json`:Bash 白名单、无 Write/Edit、`deny` Task,经 `settingsPaths` 叠在共用 hook 配置后 —— 已实现),不靠 system prompt。交接文件的写法不进 Synapse 代码 —— harness 只钉死路径 / 通道 / 标记区,推荐结构放 `synapse-tasks` repo 里的 `synapse-handoff` skill,项目 CLAUDE.md 可覆盖(标记区渲染 `renderState` 尚未实现,见落地顺序第 8 步)。
...
**主 agent(启动路径 + 受限 settings + 解绑 + 重启扫回已实现 —— 见 `docs/design/main-agent-orchestration.md` 落地顺序第 3/4 步)。** 网页直接启动 tmux 主 agent,走 `TmuxTransport` **自建会话**模式(会话名 `synapse-main-<taskId>`),**不是**接管用户 pane —— 早期「`role:'main' + transport:'tmux'` 返回 400」的约束只针对接管模式,自建会话放开。`POST /api/tasks/:id/agents/start` 的 `role:'main'` 分支:`realpathSync` work dir(避开自建会话的信任对话框坑)、已有活跃 main binding → 409、`manager.create()` 注入 `SYNAPSE_TASK_ID` / `SYNAPSE_AGENT_BINDING` / `SYNAPSE_DATA_DIR` + 基线调度者 `appendSystemPrompt` + 受限 `settingsPaths: [mainAgentSettingsPath()]`、`agent_started` 事件、`send()` 首轮 prompt。详情页「启动主 agent」对话框(work dir + model)+ 绿卡「attach」动作(复制 `tmux attach -t synapse-main-<taskId>`)。

- **受限 permissions**(§1.5):`daemon.ts` `writeMainAgentSettings()` 写 `<数据目录>/main-agent.settings.json`(`0600`,只有 `permissions`:Bash 白名单 `synapse agent:*` + git 只读 + `ls`/`cat`/`rg`/`find` + `Read(**)`;`deny` Write/Edit/NotebookEdit/Task/WebFetch/WebSearch)。经 `settingsPaths` 叠在共用 `hooks.settings.json` 之后(`--settings` 可传多个,叠加)。白名单外的 Bash 交回 Claude Code 内置权限,自建会话没人 attach 确认 → 命令挂住(有意的 fail-safe)。attach 进会话的用户手敲命令同样受限。
- **解绑 = kill-session**:`DELETE .../agents/:bindingId` 判自建主 agent binding(`tmuxName === synapse-main-<taskId>` 且无 `paneId`)则 `manager.stop(localId, true)` → `TmuxTransport.stop(true)` → `tmux kill-session`。接管 pane 的普通会话解绑仍只断绑定。
- **daemon 重启扫回**:`SessionManager.#reclaimOwnTmuxSessions()` 按会话名 `has-session` 判自建 tmux 会话在不在,在就用会话名重建 `TmuxTransport` 接回;`server.ts` `reconcileMainAgentsAfterRestart()`(await `manager.reclaimDone`)对仍 exited 的 main tmux binding 写 `agent_exited` + `detachAgent`。

**待做**:预检框的 `permissions` 预览。
...
- **主 agent 调度** — 设计见 `docs/design/main-agent-orchestration.md`。落地顺序 1–7 已完成(重命名 sweep、`TaskEvent.seq` + `synapse agent context`、主 agent 启动路径、**受限 settings**、`spawn`/`poll`/`await`、worktree 薄版本、`backend/taskDocs.ts` + `synapse-tasks` repo + `synapse agent doc`)。**待实现**:第 8 步(渲染标记区 `renderState`)、第 9 步(文档 tab + 子 agent 树)。偏差项:`turn_completed` 的改动文件列表(`turn_end` 载荷没有,要从 transcript 解析)、daemon 重启窗口内漏记 turn 的对账。
...
- **hook URL 是启动时快照,daemon 换端口 / 重启后旧会话失联** — `writeHookSettings(port)`(§3.1)把 `type: "http"` 的 URL 写死成 `http://127.0.0.1:<当时端口>/api/claude-event`;已经用 `--settings` 起的 `claude` 进程读的是那一刻的文件内容,之后 daemon 不管重写多少次、换到哪个端口(`listenWithRetry` 递增重试),对这个进程都不可见 —— 它会一直报 `ECONNREFUSED <旧端口>` 直到会话重启。**候选修法**:hook 换成 `type: "command"`,执行一个每次触发时才读 `~/.synapse/port` 现拼 URL 的脚本,而不是启动时定死。**代价**(未评估、未实现):多一次进程 fork;脚本要自己实现「发请求 → 透传决策 JSON → 超时/连不上时的行为」,必须跟 `permissions.ts` 现有的 fail-closed 语义(§2.2/§6:钩子超时或连不上 = 拒绝,不能变成放行)完全对齐,否则等于悄悄改变安全语义。
